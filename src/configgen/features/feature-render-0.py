...
class RenderHandler:
 """Processes render requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty render payload")
 return {"render": str(payload)[:200], "attempts": 0}
...
def run_render(payload, **options):
 """Module-level convenience entry point."""
 return RenderHandler().run(payload, **options)