...
class SyncHandler:
 """Processes sync requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty sync payload")
 return {"sync": str(payload)[:200], "attempts": 0}
...
def run_sync(payload, **options):
 """Module-level convenience entry point."""
 return SyncHandler().run(payload, **options)